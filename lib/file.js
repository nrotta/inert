...
internals.compressionExtensions = {
    'gzip': '.gz',
    'br': '.br'
};
...
internals.marshal = function (response, next) {
    ...
    if (!response.source.settings.lookupCompressed ||
        ...
        response.request.info.acceptEncoding === 'br' )) {

        return internals.openStream(response, response.source.path, next);
    }

    const extension = response.request.info.acceptEncoding;
    const file = response.source.path + internals.compressionExtensions[extension];
    internals.openStat(file, 'r', (err, fd, stat) => {
        ...
        response.bytes(stat.size);
        response.header('content-encoding', extension);
        response.vary('accept-encoding');

        return internals.openStream(response, file, next);
    });
};
